...
class ConvexDiffeo(nn.Module):
    ...
    def torsional_rigidity(
        ...
        for offset in offset_grid:
            for n_sources in n_sources_grid:
                torsion, rel_err = self.torsional_rigidity_(
    ...
    def torsional_rigidity_(self, n_points=20000, n_sources=300, offset=0.1):
        """
        ...
        coefs = self.gelsd_like_lstsq(M, b, rcond=1e-7)

        # Boundary error estimation
        with torch.no_grad():
            sphere = self.sample_sphere(n_points)
        ...
        return torsion.unsqueeze(0), rel_err
    ...
    def _poly_basis(self, x):
        ...
        """
        return torch.stack(
            [torch.ones(x.shape[0], device=x.device), x[:, 0], x[:, 1]],
            dim=1,
        )
    ...
    def grad_rbf_x(self, x, y, eps=1.0, rbf="thin_plate"):
        ...
        if rbf == "gaussian":
            ...
            P_grad = torch.zeros((x.shape[0], 3, x.shape[1]), device=x.device)
            P_grad[:, 1, 0] = 1
            P_grad[:, 2, 1] = 1
    ...
    def neumann_eigenvalues(
        ...
        tmp = torch.einsum("bak,bik->bia", A_phi, grad_phi)
    ...
    def dirichlet_eigenvalues(
        ...
        # While it is less mathematically justified, it avoids
        # to compute tangential jacobians
